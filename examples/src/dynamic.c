...
int indel(char character) { return 1; }
...
void print_matrix(Cell matrix[MAXSTRING + 1][MAXSTRING + 1], char* pattern,
                  char* string) {
  printf("  ");
...
void backtrack_r(Cell matrix[MAXSTRING + 1][MAXSTRING + 1], int x, int y,
                 char* s) {
  char next_char = 0;
...
void populate_matrix(Cell (*matrix)[MAXSTRING + 1], char* pattern,
                     char* string) {
...
void example_dynamic(void) {
  ...
  printf("Approximate String Matching\n");
