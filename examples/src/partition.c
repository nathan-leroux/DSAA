...
void report_partitions(Cell (*matrix)[PART_MAXSET], int* set, int set_n, int parts) {
  if (parts == 1) {
    printf("exit! parts: %d, set_n: %d\n", parts, set_n);
    outcome_print(set, 0, set_n+1);
  }
  else {
    printf("recurs! parts: %d, set_n: %d\n", parts, set_n);
    report_partitions(matrix, set, matrix[parts][set_n].parent, parts-1);
    outcome_print(set, matrix[parts][set_n].parent+1, set_n+1);
  }
}

void print_partition(Cell (*matrix)[PART_MAXSET], int* set, int set_n, int parts, bool parent) {
  // partition number
  printf("  ");
  for (int x=1; x<=parts; ++x) {
    printf("%d ", x);
