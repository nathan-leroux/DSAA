 ...
 * Dynamic Array
 *
 * dynarray_print   - helper function (unused array elements are represented
 * with '_') dynarray_append  - O(1) with occasional O(n) when array needs to be
 * extended
 *                  - can be considered O(1) as the average element will only be
 * copied twice dynarray_init    - O(1) dynarray_pop     - O(n) due to shuffling
 * of elements
 */
